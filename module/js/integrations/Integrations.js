	...
	static async pGetExpandedAddonData (
		...
		return this._INTEGRATIONS
			.pSerialAwaitFirst(async itg => {
				...
				if (out && game.settings.get(SharedConsts.MODULE_ID, ModuleSettingConsts.DEV_IS_DBG)) console.debug(...Util.LGT, `Found automation for ${ent.name} (${ent.ENG_name}) in integration: ${itg.constructor.name}`);
				return out;
			});
	}
}
